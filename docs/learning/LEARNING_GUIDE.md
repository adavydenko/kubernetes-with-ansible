...
**Схемы 3–11** перенесены в тематические главы пособия (рядом с Ansible, развёртыванием, сетью, теорией сервисов, хранением и мониторингом), чтобы каждый рисунок читался вместе с поясняющим текстом и командами. Навигация:

| № | Тема | Где открыть |
|---|------|-------------|
| 3 | Роли Ansible | [Компоненты кластера → роли](../components/README.md#Роли-ansible-и-связи-между-компонентами) |
| 4–5, 13 | Развёртывание, сеть, диагностика | [Руководство по развёртыванию](../deployment/DEPLOYMENT_GUIDE.md) — [пошаговое развёртывание](../deployment/DEPLOYMENT_GUIDE.md#Пошаговое-развертывание), [требования к сети](../deployment/DEPLOYMENT_GUIDE.md#Требования-к-сети), [устранение неполадок](../deployment/DEPLOYMENT_GUIDE.md#Устранение-неполадок) |
| 6 | Типы сервисов | [Теория → Service](../theory/K8S.md#service) |
| 7–8 | Хранилище | [Local Storage — сводка](../components/storage/LOCAL_STORAGE_SUMMARY.md#Схемы-архитектура-и-жизненный-цикл-тома) |
| 9–11 | Мониторинг | [Сводка по мониторингу](../components/monitoring/MONITORING_SUMMARY.md#Схемы-стека-мониторинга) |
| 12 | Концепция кластера | [Обзор учебного пособия](../STUDY_GUIDE_OVERVIEW.md#О-документе) |
| 14 | Безопасность и эксплуатация | [Оркестрация ресурсов](../theory/K8S_ORCHESTRATION.md#Связь-безопасности-производительности-и-эксплуатации) |
| 15 | Расширенный контур тем | [Справочные материалы](../reference/README.md#Карта-смежных-тем-расширенный-контур) |
| 16 | Сертификации | [Дополнительные ресурсы → сертификация](../reference/RESOURCES.md#Сертификация) |
...
Схемы сети и типов сервисов (диаграммы 5–6) см. в [руководстве по развёртыванию](../deployment/DEPLOYMENT_GUIDE.md#Требования-к-сети) и в [теории, раздел Service](../theory/K8S.md#service).
...
Схемы архитектуры и жизненного цикла тома (диаграммы 7–8) — в [сводке по локальному хранилищу](../components/storage/LOCAL_STORAGE_SUMMARY.md#Схемы-архитектура-и-жизненный-цикл-тома).
...
Схемы стека мониторинга (диаграммы 9–11) — в [сводке по мониторингу](../components/monitoring/MONITORING_SUMMARY.md#Схемы-стека-мониторинга).

Ниже переходим к практическим лабораторным работам.
...
При возникновении проблем в кластере важно следовать систематическому подходу к диагностике. Пошаговый алгоритм (диаграмма 13) вынесен в [раздел «Устранение неполадок» руководства по развёртыванию](../deployment/DEPLOYMENT_GUIDE.md#Устранение-неполадок); ниже — компактная шпаргалка команд.
...
Для успешной эксплуатации кластера важно следовать установленным принципам и практикам. Взаимосвязь безопасности, производительности и операций (диаграмма 14) раскрыта в главе [«Оркестрация Kubernetes»](../theory/K8S_ORCHESTRATION.md#Связь-безопасности-производительности-и-эксплуатации).
...
После освоения базовых концепций Kubernetes вы можете углубиться в более продвинутые темы. Наглядная [карта смежных направлений](../reference/README.md#Карта-смежных-тем-расширенный-контур) (диаграмма 15) вынесена в справочный раздел.
...
Для тех, кто планирует получить официальные сертификации Kubernetes, важно понимать структуру экзаменов и темы, которые они покрывают. [Карта сертификаций](../reference/RESOURCES.md#Сертификация) (диаграмма 16) и ссылки на программы экзаменов — в разделе «Сертификация» на странице дополнительных ресурсов.
...
Поздравляем! Вы успешно прошли полный курс по развертыванию и управлению Kubernetes кластером. В ходе обучения мы изучили теоретические основы, использовали наглядные схемы (см. навигацию в начале пособия и [индекс в конце](#-Индекс-диаграмм-и-примеров-кода)), выполнили практические лабораторные работы и освоили лучшие практики.
...
| 3 | Структура Ansible ролей | [Компоненты кластера → роли](../components/README.md#Роли-ansible-и-связи-между-компонентами) | Структура ролей и зависимости | kubernetes_master, kubernetes_worker, metallb |
| 4 | Процесс развертывания кластера | [Развёртывание → пошаговое развёртывание](../deployment/DEPLOYMENT_GUIDE.md#Пошаговое-развертывание) | Пошаговый процесс развертывания | Подготовка, установка, настройка сети |
| 5 | Сетевая архитектура | [Развёртывание → требования к сети](../deployment/DEPLOYMENT_GUIDE.md#Требования-к-сети) | Полная сетевая архитектура кластера | Flannel, MetalLB, внешний доступ |
| 6 | Типы сервисов Kubernetes | [Теория → Service](../theory/K8S.md#service) | Различные типы сервисов | ClusterIP, NodePort, LoadBalancer |
| 7 | Архитектура хранилища | [Хранилище → схемы](../components/storage/LOCAL_STORAGE_SUMMARY.md#Схемы-архитектура-и-жизненный-цикл-тома) | Архитектура локального хранилища | Local Storage Provisioner, PV/PVC |
| 8 | Жизненный цикл PersistentVolume | [Хранилище → схемы](../components/storage/LOCAL_STORAGE_SUMMARY.md#Схемы-архитектура-и-жизненный-цикл-тома) | Жизненный цикл PV | Создание, binding, использование, очистка |
| 9 | Архитектура мониторинга | [Мониторинг → схемы](../components/monitoring/MONITORING_SUMMARY.md#Схемы-стека-мониторинга) | Архитектура системы мониторинга | Prometheus, Grafana, Node Exporter |
| 10 | Поток данных мониторинга | [Мониторинг → схемы](../components/monitoring/MONITORING_SUMMARY.md#Схемы-стека-мониторинга) | Поток данных в системе мониторинга | Сбор, хранение, визуализация, алерты |
| 11 | Компоненты мониторинга | [Мониторинг → схемы](../components/monitoring/MONITORING_SUMMARY.md#Схемы-стека-мониторинга) | Компоненты и их взаимосвязи | Data Sources, Core Monitoring, Alerting |
| 12 | Концептуальная схема кластера | [Обзор пособия](../STUDY_GUIDE_OVERVIEW.md#О-документе) | Концептуальная схема всего кластера | Control Plane, Data Plane, Networking |
| 13 | Процесс диагностики проблем | [Развёртывание → устранение неполадок](../deployment/DEPLOYMENT_GUIDE.md#Устранение-неполадок) | Алгоритм устранения неполадок | Диагностика узлов, сети, сервисов |
| 14 | Принципы безопасности | [Оркестрация → связь тем](../theory/K8S_ORCHESTRATION.md#Связь-безопасности-производительности-и-эксплуатации) | Взаимосвязи безопасности и производительности | Security, Performance, Operations |
| 15 | Карта расширенных технологий | [Справочник](../reference/README.md#Карта-смежных-тем-расширенный-контур) | Карта технологий и взаимосвязей | Security, Scaling, Configuration Management |
| 16 | Карта сертификаций | [Ресурсы → сертификация](../reference/RESOURCES.md#Сертификация) | Карта сертификаций и тематическое покрытие | CKA, CKAD, CKS |
