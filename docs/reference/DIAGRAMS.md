...
> В текущем учебном пособии ключевые диаграммы вставлены непосредственно в содержимое тематических разделов, чтобы их можно было изучать без перехода на отдельные страницы.
>
> Актуальные исходники и сгенерированные SVG находятся в [`../diagram-assets/`](../diagram-assets/README.md).
...
Диаграммы можно интегрировать в документацию как встроенные изображения:

```markdown
![Kubernetes Architecture](diagrams/kubernetes-architecture.png)
```

Рекомендуется размещать визуализацию сразу после абзаца, где эта схема впервые упоминается.
